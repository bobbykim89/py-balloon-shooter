...
enemy_width = enemy_size[0] ## horizontal size of enemy
enemy_height = enemy_size[1] ## vertical size of enemy
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) ## place enemy in middle of horizontal screen width
enemy_y_pos = (screen_height / 2) - (enemy_height / 2) ## place enemy in bottom of screen vertical height

## Config font
game_font = pygame.font.Font(None, 40) # create font entity (font, size)

## Total time
total_time = 10

## Initial time info
start_ticks = pygame.time.get_ticks() # get current tick
...
while running:
    ...
    screen.blit(enemy, (enemy_x_pos, enemy_y_pos)) # draw an enemy character

    ## Insert timer
    # calc elapsed time
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # convert elapsed time to sec from ms

    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255))
    # output text, True, text color
    screen.blit(timer, (10, 10))

    ## exit game once time is below 0
...
## Stationary for short time
pygame.time.delay(2000) # wait for 2 sec
